    ...
    formulario.addEventListener("submit", function(e){
       ...
       let errores =[];
        ...
        let campoSku = document.querySelector ("input.sku")
            if( campoSku.value == "") {
            errores.push("*SKU: Tenes que ingresar un numero de 5 cifras. Ej: 0000.1");
            }
            else if(campoSku.value.length < 4) {
        ...
        let campoBase = document.querySelector ("input.base")
            if( campoBase.value == "") {
                errores.push("*BASE: Tenes que ingresar detalles del tipo de base. Ej: Al agua - Al aceite - Al aguarras.busqueda" );
                        }
        ...
        let campoRendimiento = document.querySelector ("input.rendimiento")
            ...
            else if(campoRendimiento.value.length < 3) {
            errores.push("*RENDIMIENTO: Tenes que ingresar detalles del rendimiento. Ejemplo 12 a 16 m2/l por mano.");
            };               
        ...
        let campoTiempoSecado = document.querySelector ("input.tiempo_de_secado")
            ...
            else if(campoTiempoSecado.value.length < 3) {
            errores.push("*TIEMPO DE SECADO: Tenes que ingresar detalles tiempo de secado: Ej: 30 minutos- 4 horas.");
            };   
        let campoGarantia = document.querySelector ("input.garantia")
            if ((campoGarantia.value == "") ) {
        ...
        let campoTitulo = document.querySelector ("textarea.titulo_producto")
            if( campoTitulo.value == "") {
            errores.push("*TITULO: Tenes que agregar un título de producto." );
            }  
            else if(campoGarantia.value.length < 3) {
                errores.push("*TITULO: Tenes que agregar un título de producto." );
                };                      
        let campoDescripcion = document.querySelector ("textarea.descripcion")
            ...
            errores.push("*TITULO: Tenes que agregar una descripción del producto." );
            }
            else if(campoGarantia.value.length < 3) {
                errores.push("*TITULO: Tenes que agregar una descripción del producto." );
                };     
